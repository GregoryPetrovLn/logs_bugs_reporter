...
public class BugsOpeningService {
	...
	void bugOpening(LogDto logException) {
		...
		Bug bug = new Bug(description, LocalDate.now(), null, bugStatus, seriousness, OpenningMethod.AUTOMATIC,
				programmer);
		bugsRepo.save(bug);
		LOG.debug(
				"Opening service has added bug with description: {}," + " status: {}, seriousness: {}, assigned to {} ",
				description, bugStatus, seriousness, programmer == null ? "no assigned" : programmer.getName());
