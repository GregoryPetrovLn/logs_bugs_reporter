...
public class FillDatabase {
	...
	@Value("${app-programmer-names:Moshe," + "David,Avraham,Kseniya,Sergey,Yosef,"
			+ "Sara,Vasya,Asher,Yuri,Igor,Benyamin," + "Genady,Mariya,Sonya,Nikolay, Amir}")
	String names[];
	...
	private void fillArtifacts() {
		for (String artifactId : artifacts) {
			reporterService.addArtifact(
					new ArtifactDto(artifactId, ThreadLocalRandom.current().nextLong(1, names.length + 1)));
		}
		for (int i = 1; i <= nArtifactsNofixed; i++) {
			reporterService.addArtifact(
					new ArtifactDto(artifactBase + i, ThreadLocalRandom.current().nextLong(1, names.length + 1)));
		}
	...
	private void fillProgrammers() {
		for (int i = 0; i < names.length; i++) {
			...
			reporterService.addProgrammer(new ProgrammerDto(programmerId, names[i], getEmail(programmerId)));
		}
