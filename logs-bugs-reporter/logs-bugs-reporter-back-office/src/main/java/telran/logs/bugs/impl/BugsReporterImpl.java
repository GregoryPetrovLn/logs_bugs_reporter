...
public class BugsReporterImpl implements BugsReporter {
    ...
    public ProgrammerDto addProgrammer(ProgrammerDto programmerDto) {
        ...
        if (programmerRepository.findById(programmerDto.id).orElse(null) != null) {
    ...
    public ArtifactDto addArtifact(ArtifactDto artifactDto) {
        if (programmerRepository.findById(artifactDto.programmerId).orElse(null) == null) {
            throw new NotFoundException(String.format("programmer with id:%s not found", artifactDto.programmerId));
        }
    ...
    public void assignBug(AssignBugData assignData) {
        if (bugRepository.findById(assignData.bugId).orElse(null) == null) {
            throw new NotFoundException(String.format("Bug with id:%s not found", assignData.bugId));
        }
        if(programmerRepository.findById(assignData.programmerId).orElse(null) == null){
            throw new NotFoundException(String.format("Programmer with id:%s not found", assignData.programmerId));
        }
    ...
    public void closeBug(CloseBugData closeData) {
        ...
        if (bug == null || bug.getStatus() == BugStatus.CLOSED) {
            throw new NotFoundException(String.format("Bug with id:%s not found", closeData.bugId));
        }
